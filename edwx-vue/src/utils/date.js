...
const DateUtils = {
  ...
  formatTimeStamp: function (date) {
    // console.log(666,moment(date).format('L'))
    return moment(date).format('L');
  },

  getWeekTimeStamp: function (date) {     //根据时间戳获取当前是周几
    // console.log(666,moment(date).day())
    return moment(date).day();
  },

  getTimeStamp: function (date) {     //根据当前时间获取当前时间戳
    // console.log(666,moment(date).day())
  ...
  getWeekDaysCn: function (date, days) {
    if (days == undefined || days == '') {
      days = 0;
    }
  ...
  getWeekCn: function (date) {
    return moment(date, _fmt).format('ddd');
  },

  getWeekCnOne: function (date) {
    return moment(date).format('dd');
  },

  getWeekCnThree: function (date) {
    return moment(date, _fmt).format('dddd');
  },

  getMonthMM: function (date) {
  ...
  getMonthEnd: function (date, fmt = 'L') {
    return moment(date, _fmt).endOf('month').format(fmt);
  },

  getMonthStart: function (date, fmt = 'L') {
    return moment(date, _fmt).startOf('month').format(fmt);
  },

  getNextMonthStart: function (date, months) {
    if (months == undefined || months == '') {
      months = 0;
    }
  ...
  getMMDDHHmmss: function (date) {
    return moment(date, _fmt).format('MM-DD HH:mm:ss');
  },

  getYYYYMMDDHHmm: function (date) {
  ...
  formatScmDate: function (date) {
    return (Number(date) / 60 < 10 ?
      0 + '' + Number(date) / 60 :
      Number(date) / 60) +
      ':' +
      (Number(date) % 60 < 10 ?
  ...
  formatScmTime: function (time) {
    let obj = time.split(':');
    let temp = '';
    for (let idx in time.split(':')) {
      temp = temp + ':' + (Number(obj[idx]) < 10 ?
        0 + '' + Number(obj[idx]) :
        Number(obj[idx]));
    }
  ...
  getMinutesStamp: function (date) {
    return moment.duration(date).asMinutes();
  },

  addDateTime: function (date, days) {
    if (days == undefined || days == '') {
      days = 0;
    }
    return moment(date, _fmt).add(days, 'days').format('L');
  },
  addMinuteTime: function (time, val) {
    return moment(time).add(val, 'minutes').format('YYYY-MM-DD HH:mm');
  },
  subtractMinuteTime: function (time, val) {
