<template>
    <div class="repairAttendance-detail">
        ...
        <div class="detail-info">
            <group class="apply-boxs">
                ...
                <cell title="补出勤日期" value-align="left" :value="repairDay"></cell>        <!--后台未返回此字段-->
                <cell title="上班漏打卡" value-align="left" :value="apply.sbsj.substring(11,16)" v-if="apply.sbsj"></cell>
                <cell title="下班漏打卡" value-align="left" :value="apply.xbsj.substring(11,16)" v-if="apply.xbsj"></cell>
                <cell class="reason" title="补录事由" value-align="left">
...
<script>
    ...
    export default {
        ...
        methods: {
                    ...
                    userSerial: this.$utils.Store.getItem('userSerial'),
                        ...
                        if (+code !== this.$code.success) {
                            ...
                            if (data.midData.sbsj) {
                                this.repairDay = data.midData.sbsj.substring(0, 10);
                            } else {
                                this.repairDay = data.midData.xbsj.substring(0, 10);
                            }
            ...
            onConfirm() {
                this.$utils.Tools.loadShow();
                this.$api.Repair.deleteRepair(this.xh, this.$utils.Store.getItem('userSerial'), '').then(response => {
                        this.$utils.Tools.loadHide();
                        // console.log(2222, response)
                        const {code, msg, data} = response;
                        if (+code !== this.$code.success) {
