<template>
  <div class="overtime" :class="showFlag?'overf':''">
    <group class="apply-boxs" label-width="4.5em" label-margin-right="1em">
      <popup-picker class="work-type" title="加班类型" :data="workTypeList" :columns="1" v-model="workType"
                    :show-name="true" placeholder="请选择" @on-change="changeWorkType"></popup-picker>
      <datetime title="开始时间" v-model="kssj" format="YYYY-MM-DD HH:mm" :max-hour=23
                placeholder="选择时间" @on-change="changeStartTime"></datetime> <!--:minute-list="minuteList"-->
      <datetime title="结束时间" v-model="jssj" format="YYYY-MM-DD HH:mm"
                :max-hour=23 placeholder="选择时间" @on-change="changeEndTime"></datetime><!--:minute-list="minuteList"-->
      <x-textarea class="reason" :max="1000" name="description" placeholder="加班事由（最多输入1000字）" v-model="reason"
...
<script>
  ...
  export default {
    name: 'Overtime',
    components: {
      Group, Cell, XInput, Selector, PopupPicker, Datetime, OrgTree, XTextarea, XSwitch, XButton, UploadImage,
    },
    beforeRouteLeave(to, from, next) {
      if (this.showFlag) {
        ...
        this.$utils.Store.getItem('workType');
        next();
      }
    },
    data() {
      return {
        ...
        workType: this.$utils.Store.getItem('workType') ? JSON.parse(this.$utils.Store.getItem('workType')) : [],
        tripId: '',    //会议室编号
        startDate: '',   //开始日期
        endDate: '',     //截止日期
    ...
    computed: {
      isDisabled() {
        if (this.kssj != '' && this.jssj != '' && this.reason != '' && this.workType.length > 0 && this.approver.length > 0) {    /**/
          return false;
        } else {
          return true;
        }
    ...
    methods: {
      ...
      changeWorkType(val) {
        this.workType = val;
        this.$utils.Store.setItem('workType', JSON.stringify(val));
        console.log('val change', val, this.$utils.Store.getItem('workType'));
      },
      /** function 获取外出开始时间
       *   wzx
       *   2019-01-05
      ...
      selectCopyUser() {
        this.$api.Copyer.selectCopyUser({
          lx: 3,    //0:请假  1:出差  2:外出  3:加班  4:调班  5:补出勤 6:合同
          userSerial: this.$utils.Store.getItem('userSerial'),
        }).then(response => {
            const {code, msg, data} = response;
      ...
      submitClick() {
        if (this.kssj > this.jssj) {
          ...
          let workType = JSON.stringify(this.workType).replace(/\[|]|"/g, '');
          ...
          // console.log(666,workType)
          this.$utils.Tools.loadShow();
          this.$api.Overtime.insertOverTime({
            bh: this.bh,
            bz: this.reason,
            imgPath: imgPath,
            kssj: this.kssj,
