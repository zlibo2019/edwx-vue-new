...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onConfirm() {
                this.$utils.Tools.loadShow();
                this.$api.Out.deleteOut(this.xh, this.$utils.Store.getItem('userSerial'), '').then(response => {
                        this.$utils.Tools.loadHide();
                        // console.log(2222, response)
                        const {code, msg, data} = response;
                        if (+code !== this.$code.success) {
        ...
        mounted() {
            // console.log(666,this.$route.params.notification)
