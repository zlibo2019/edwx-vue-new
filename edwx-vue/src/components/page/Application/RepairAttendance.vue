<template>
  <div class="repair-attendance" :class="showFlag?'overf':''">
    <group class="apply-boxs" label-width="8em" label-margin-right="-2em" label-align="left">
      <datetime title="补出勤日期" v-model="repairDate" format="YYYY-MM-DD" placeholder="选择日期"
                @on-change="changeRepairDate"></datetime>
      <cell class="myset" v-if="type==='0' || type==='2'">
        <check-icon :value.sync="onValue" v-if="type==='2'" slot="title">上班漏打卡</check-icon>
        <span slot="title" v-if="type==='0'">上班漏打卡</span>
        <datetime slot="default" class="timeset" title="" v-model="onTime" format="HH:mm"
                  placeholder="选择时间" @on-change="changeOnTime" :readonly="checkOn"></datetime>
      </cell>
      <cell class="myset" v-if="type==='1' || type==='2'">
        <check-icon :value.sync="offValue" v-if="type==='2'" slot="title">下班漏打卡</check-icon>
        <span slot="title" v-if="type==='1'">下班漏打卡</span>
        <datetime slot="default" class="timeset" title="" v-model="offTime" format="HH:mm"
                  placeholder="选择时间" @on-change="changeOffTime" :readonly="checkOff"></datetime>
      </cell>
      <x-textarea class="reason" :max="1000" name="description" placeholder="补录事由（最多输入1000字）"
                  v-model="reason" @on-change="getReason"></x-textarea>
    </group>
...
<script>
  import {
    Group, Cell, XInput, Datetime, XTextarea, XButton, CheckIcon,
  } from 'vux';
  ...
  export default {
    ...
    components: {
      Group, Cell, XInput, Datetime, XTextarea, XButton, CheckIcon, OrgTree, UploadImage,
    },
    beforeRouteLeave(to, from, next) {
      if (this.showFlag) {
        ...
        this.$utils.Store.removeItem('onTime');
        this.$utils.Store.removeItem('offTime');
        this.$utils.Store.removeItem('repairDate');
    ...
    data() {
      return {
        ...
        bh: '00203',
        repairDate: this.$utils.Store.getItem('repairDate') ? this.$utils.Store.getItem('repairDate') : '',  //补出勤日期
        startDate: '',   //今天日期
        onTime: this.$utils.Store.getItem('onTime') ? this.$utils.Store.getItem('onTime') : '',   //上班时间
        offTime: this.$utils.Store.getItem('offTime') ? this.$utils.Store.getItem('offTime') : '',   //下班时间
        onValue: this.$utils.Store.getItem('onTime') ? true : false,    //上班补录设置
        offValue: this.$utils.Store.getItem('offTime') ? true : false,    //下班补录设置
        reason: this.$utils.Store.getItem('reason') ? this.$utils.Store.getItem('reason') : '',   //补录事由
        ...
        type: this.$utils.Store.getLocalItem('repairAttendanceType') || '2',
    ...
    computed: {
      isDisabled() {
        let flag = false;
        if (this.repairDate !== '' && this.reason !== '' && this.approver.length > 0 && (this.onValue || this.offValue)) {    /**/
          if (this.onValue) {
            flag = this.onTime === '';
          }
    ...
    methods: {
      getSetting() {
        this.$api.Sys.getSystemParams(0, this.$constant.repairAttendanceType).then(response => {
          ...
          if (+code !== this.$code.success) {
            ...
            if (data === null) {
              ...
              this.$utils.Store.setLocalItem('repairAttendanceType', data.paramValue);
            }
          }
        }).catch(err => {
      ...
      changeRepairDate(val) {
        this.repairDate = val;
        this.$utils.Store.setItem('repairDate', val);
      },
      /** function 获取上班时间
       *   wzx
       *   2019-01-11
      ...
      changeOnTime(val) {
        this.onTime = val;
        this.$utils.Store.setItem('onTime', val);
        if (this.offTime != '' && this.onTime > this.offTime) {
          this.$utils.Tools.toastShow('上班时间不能大于下班时间');
          setTimeout(() => {
            this.onTime = '';
          }, 2000);
        }
      ...
      changeOffTime(val) {
        this.offTime = val;
        this.$utils.Store.setItem('offTime', val);
        if (this.onTime != '' && this.offTime < this.onTime) {
          this.$utils.Tools.toastShow('下班时间不能小于上班时间');
          setTimeout(() => {
            this.offTime = '';
          }, 2000);
        }
      ...
      selectCopyUser() {
        this.$api.Copyer.selectCopyUser({
          lx: 5,    //0:请假  1:出差  2:外出  3:加班  4:调班  5:补出勤 6:合同
          userSerial: this.$utils.Store.getItem('userSerial'),
        }).then(response => {
            const {code, msg, data} = response;
      ...
      submitClick() {
        ...
        this.$utils.Tools.loadShow();
        this.$api.Repair.insertRepair({
          bh: this.bh,
          bz: this.reason,
          imgPath: imgPath,
          sbsj: this.$utils.Store.getItem('onTime') ? this.repairDate + ' ' + this.onTime : '',
          xbsj: this.$utils.Store.getItem('offTime') ? this.repairDate + ' ' + this.offTime : '',
          processId: '',
          qjlx: 1,
          userSerial: this.$utils.Store.getItem('userSerial'),
          xh: '',
