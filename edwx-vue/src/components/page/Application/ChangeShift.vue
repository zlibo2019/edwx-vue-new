<template>
  <div class="change-shift" :class="showFlag?'overf':''">
    <group class="apply-boxs" label-width="4.5em" label-margin-right="1em">
      <datetime title="调班时间" v-model="changeDate" format="YYYY-MM-DD" placeholder="选择时间"
                @on-change="changeDay"></datetime>
      <x-input title="原班次" v-model="oldClassesName" text-align="right" readonly></x-input>
      <popup-picker class="shift-type" title="新班次" :data="classesList" :columns="1" v-model="newClasses"
                    :show-name="true" placeholder="请选择" @on-change="changeShift"></popup-picker>
      <x-textarea class="reason" :max="1000" name="description" placeholder="调班事由（最多输入1000字）" v-model="reason"
  ...
  export default {
    ...
    data() {
      return {
        ...
        oldClassesName: this.$utils.Store.getItem('oldClassesName') ? this.$utils.Store.getItem('oldClassesName') : '',    //原班次名称
        oldClasses: this.$utils.Store.getItem('oldClasses') ? this.$utils.Store.getItem('oldClasses') : '',    //原班次编号
        startDate: '',   //今天日期
        changeDate: this.$utils.Store.getItem('changeDate') ? this.$utils.Store.getItem('changeDate') : '',   //调班时间
        endTime: '',   //开始时间
    ...
    methods: {
      ...
      getBanci() {
        this.$api.ChangeShift.getBanci({
          ...
          if (+code !== this.$code.success) {
            ...
            this.oldClassesName = data.banci.mc;
            this.oldClasses = data.banci.bh;
            this.$utils.Store.setItem('oldClassesName', data.banci.mc);
            this.$utils.Store.setItem('oldClasses', data.banci.bh);
            let allBC = data.BanciAll;
            for (let i = 0; i < allBC.length; i++) {
              this.classesList.push({name: allBC[i].mc, value: allBC[i].bh});
            }
          }
        }).catch(err => {
          this.$utils.Tools.httpError();
      ...
      selectCopyUser() {
        this.$api.Copyer.selectCopyUser({
          lx: 4,    //0:请假  1:出差  2:外出  3:加班  4:调班  5:补出勤 6:合同
          userSerial: this.$utils.Store.getItem('userSerial'),
        }).then(response => {
          const {code, msg, data} = response;
